...
import { farmerSignIn } from "@/services/api";
...
export default function FarmerSignIn() {
  ...
  const mutation = useMutation({
    mutationFn: (data: SignInForm) => farmerSignIn(data.email, data.password),
    onSuccess: (data) => {
      signIn({ farmer: data.seller, accessToken: data.accessToken });
      router.replace("/(farmer)/(main)");
    },
    onError: (error: any) => {
      ...
      >
        ...
        >
          <TitleSection
            ...
            description="Sign in to continue to your farmer account"
          />
          <Controller
            control={control}
            name="email"
          ...
          />
          <FooterLink
            label="Don't have a farmer account?"
            linkText="Sign Up"
            href="/(farmer)/auth/sign-up"
          />
          <FooterLink
